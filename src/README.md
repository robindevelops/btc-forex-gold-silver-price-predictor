...
| Package | File | Role |
|---|---|---|
| `data/` | `data_collection.py` | Yahoo Finance OHLCV for BTC-USD, GC=F, SI=F (`--start` for longer history); complete bars only |
| | `market_calendar.py` | complete-bar rule (crypto: UTC day; futures / US series: 17:15 ET) and next-trading-day helper |
...
Data-flow: `data/raw → preprocessing → build_dataset → {tune, train, stack} → backtesting → results/ → {streamlit, api}`; live: `Yahoo → data/raw_live → live features → prediction.predict_next_day (Combined)`.
