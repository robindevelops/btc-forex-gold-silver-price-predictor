...
Layout
  Overview strip            what is predicted · served model · reliability on the unseen test period
  1 Forecast                latest market data · next-day prediction · forecast chart · price history (train/val/test)
  2 Predict a Day           stand on any unseen test day, predict, reveal the actual close and the error
  3 Prediction History      every unseen day: predicted vs actual, error, direction hit (CSV download)
  4 Model Performance       test table · honest reading · comparison chart · walk-forward table · predicted vs actual
  5 Methodology             target, data, split, features, models, served-model details
"""
import os
import sys
import logging
...
from src.models.registry import RECURRENT
...
def test_row(te: Optional[pd.DataFrame], asset: str, model: str) -> Optional[pd.Series]:
    """The selected model's single test-set evaluation (results/final_test_results.csv)."""
    if te is None:
        return None
...
# ─────────────────────────────────────────────── sidebar: the only place the asset and model are chosen
...
served = status.get('primary_model')
models = available_models(asset)
model = st.sidebar.selectbox("Model", models or ['(no trained models)'],
                             index=models.index(served) if served in models else 0,
                             help="The served model is the walk-forward validation winner; the others are comparison rows.")
if _qp.get('run') == '1':
    st.session_state['run_prediction'] = True
if st.sidebar.button("Run next-day prediction", type="primary", use_container_width=True):
    st.session_state['run_prediction'] = True
...
sel_test = test_row(te_all, asset, model)
naive_test = test_row(te_all, asset, 'Naive')

input_desc = f"last {SEQ_LEN} days of features" if model in RECURRENT else "today's features (one row)"
st.sidebar.markdown("---")
st.sidebar.caption(f"**Served model:** {served or 'n/a'}  \n**Selected model input:** {input_desc}  \n"
                   f"**Horizon:** {PREDICTION_HORIZON_DAYS} trading day  \n**Selected by:** walk-forward validation (train+val only)  \n"
                   f"**Data:** Yahoo Finance · {df.attrs.get('source', '—') if df is not None else '—'} · last close "
                   f"{df['timestamp'].iloc[-1].date() if df is not None else '—'}")

# ─────────────────────────────────────────────── title + overview strip
st.title(f"{asset} — Next-Day Price Forecast")
tm = status.get('test') or {}; tn = status.get('test_naive') or {}
if tm:
    ...
    st.markdown(
        ...
        f"(\"tomorrow = today\"). <b>The model does not beat the random walk</b> (Diebold–Mariano p = {tm['DM_pvalue']:.2f})."
        f"</div>", unsafe_allow_html=True)
else:
    st.info("No evaluation found. Run `python src/evaluation/backtesting.py` first.")

tab_fc, tab_demo, tab_hist, tab_perf, tab_meth = st.tabs(
    ["Forecast", "Predict a Day (unseen test)", "Prediction History", "Model Performance", "Methodology"])
...
with tab_fc:
    if df is None or len(df) < 2:
        ...
        c1.metric(f"Close on {latest_date}", usd(latest_price),
        ...
        if not st.session_state['run_prediction'] or not models:
            st.caption("Choose a model in the sidebar and press **Run next-day prediction**.")
        else:
            try:
                with st.spinner(f"Running {model}…"):
                    r = predict_next_day(asset, model)
                rmse_ret = float(sel_test['RMSE_ret']) if sel_test is not None else None
                r_hat = r['predicted_return_pct'] / 100
                band = ([r['current_price'] * np.exp(r_hat - rmse_ret), r['current_price'] * np.exp(r_hat + rmse_ret)]
                        if rmse_ret is not None else None)
                p1, p2, p3, p4 = st.columns(4)
                p1.metric(f"Close on {r['as_of_date']}", usd(r['current_price']))
                p2.metric("Predicted next close", usd(r['predicted_price']),
                          f"{r['predicted_price'] - r['current_price']:+,.2f} ({r['predicted_return_pct']:+.2f}%)")
                p3.metric("Direction", "UP ▲" if r['direction'] == 'UP' else "DOWN ▼")
                p4.metric(f"Model ({'served' if r['is_served_model'] else 'comparison'})", model)
                if band:
                ...
                tail = df.tail(60)
                next_date = (tail['timestamp'].iloc[-1] + pd.tseries.offsets.BDay(1)) if asset != 'Bitcoin' \
                    else (tail['timestamp'].iloc[-1] + pd.Timedelta(days=1))
                fh = go.Figure()
                ...
                # reliability of the selected model (from the single test-set evaluation)
                if sel_test is not None and naive_test is not None:
                    st.markdown(f"**How reliable is {model}?** Single evaluation on {int(sel_test['n_test'])} unseen days "
                                f"({status['test_period'][0]} → {status['test_period'][1]}), side by side with the random walk (\"tomorrow = today\").")
                    beats = sel_test['DM_pvalue'] < 0.05 and sel_test['RMSE_ret'] < naive_test['RMSE_ret']
                    rel_tbl = pd.DataFrame({
                        '': [model, 'Random walk'],
                        'Mean abs. error': [usd(sel_test['MAE_usd']), usd(naive_test['MAE_usd'])],
                        'RMSE': [usd(sel_test['RMSE_usd']), usd(naive_test['RMSE_usd'])],
                        'MAPE': [pct(sel_test['MAPE_usd'], 2), pct(naive_test['MAPE_usd'], 2)],
                        'Direction correct': [f"{pct(sel_test['DirAcc_pct'])} (p = {sel_test['DirAcc_pvalue']:.2f} vs 50%)", '— (no direction)'],
                        'Beats random walk?': [f"{'Yes' if beats else 'No'} (Diebold–Mariano p = {sel_test['DM_pvalue']:.2f})", '—']})
                    st.dataframe(rel_tbl, use_container_width=True, hide_index=True)

                if len(models) > 1:
                    with st.expander("Compare all trained models on the same input"):
                        ...
                        if rows:
                            ...
                            t['Role'] = t['model'].map(lambda m: 'served' if m == served else '')
                            st.dataframe(t[['model', 'Predicted close', 'Change', 'Direction', 'Role']].rename(columns={'model': 'Model'}),
                                         use_container_width=True, hide_index=True)
                            st.caption("The models agree to within a fraction of a percent because daily returns are mostly noise: "
                                       "every model has learned to stay close to \"no change\".")
...
with tab_demo:
    ...
    st.markdown(f"Stand on any day after **{VAL_END}** — a period never used for training, validation or model selection. "
                f"The model sees data **only up to that day**, predicts the next trading day's close, and the actual close is then revealed.")
    if hist is None or df is None:
        ...
        with c2:
            ...
            go_demo = st.button("Generate prediction", type="primary", use_container_width=True)
        if go_demo or st.session_state.get('demo_done'):
            st.session_state['demo_done'] = True
            try:
                r = predict_for_date(asset, str(pick), model)
                ...
                st.caption(f"Model: **{r['model_used']}** ({'served' if r['model_used'] == served else 'comparison'}) · horizon {r['horizon_days']} trading day · "
                ...
                col_pred = f'pred_close_{model}'
                if col_pred in hist.columns:
                    fd = go.Figure()
                    fd.add_trace(go.Scatter(x=hist['target_date'], y=hist['actual_close'], mode='lines', name='actual close',
                                            line=dict(color=COLOR[asset], width=2)))
                    fd.add_trace(go.Scatter(x=hist['target_date'], y=hist[col_pred], mode='lines', name=f'{model} predicted close',
                    ...
                    st.plotly_chart(layout(fd, 400, f"Unseen test period {hist['target_date'].min().date()} → {hist['target_date'].max().date()}: "
                                                    f"actual vs predicted close ({model})"), use_container_width=True)
                    st.caption("A predicted-price line always hugs the actual line because it starts from today's close. "
                               "Whether the model has skill is only visible in return space — see Model Performance.")

                rows = predict_all_models(asset, str(pick))
                if rows:
                    t = pd.DataFrame(rows)
                    t['Predicted'] = t['predicted_price'].map(usd)
                    t['Actual'] = t['actual_price'].map(usd)
                    t['Error'] = t['error_pct'].map(lambda v: f"{v:+.2f}%" if pd.notna(v) else "—")
                    t['Role'] = t['model'].map(lambda m: 'served' if m == served else '')
                    st.markdown("**All trained models on this day**")
                    st.dataframe(t[['model', 'Predicted', 'Actual', 'Error', 'Role']].rename(columns={'model': 'Model'}),
                                 use_container_width=True, hide_index=True)
            except Exception as e:
                st.error(f"Prediction failed: {e}")
...
with tab_hist:
    st.subheader("Prediction history — every unseen test day")
    if hist is None or f'pred_close_{model}' not in hist.columns:
        st.warning("Run `python src/evaluation/backtesting.py` first.")
    else:
        e_m, e_n = hist[f'error_pct_{model}'].abs(), hist['error_pct_Naive'].abs()
        hits = int(hist[f'direction_hit_{model}'].sum())
        st.caption(f"{len(hist)} unseen days, {hist['target_date'].min().date()} → {hist['target_date'].max().date()}. Summary of the table below:")
        st.dataframe(pd.DataFrame({'': [model, 'Random walk'],
        ...
        h = pd.DataFrame({'Predicted on': hist['date'].dt.date, 'For': hist['target_date'].dt.date,
                          'Predicted ($)': hist[f'pred_close_{model}'].round(2), 'Actual ($)': hist['actual_close'].round(2),
                          'Error ($)': (hist[f'pred_close_{model}'] - hist['actual_close']).round(2),
                          'Error (%)': hist[f'error_pct_{model}'].round(2),
                          'Random walk error (%)': hist['error_pct_Naive'].round(2),
                          'Direction': hist[f'direction_hit_{model}'].map({1: 'hit', 0: 'miss'})})
        st.dataframe(h.sort_values('For', ascending=False), use_container_width=True, hide_index=True, height=440,
        ...
        st.download_button("Download history (CSV)", h.to_csv(index=False).encode(), file_name=f"{asset.lower()}_{model.lower()}_prediction_history.csv")
        st.caption(f"Every row is an out-of-sample prediction: the model was trained on data up to {VAL_END} and each prediction used "
...
with tab_perf:
    if cv_all is None or te_all is None or not tm:
        ...
        st.caption(f"Test period {s['test_period'][0]} → {s['test_period'][1]} ({s['test']['n_test']} days). Served model **{served}**, "
                   f"chosen by {s.get('selection_rule', 'walk-forward validation')} — the test set never influenced selection.")
        t = te_all[te_all['asset'] == asset].sort_values('RMSE_ret')
        is_naive = (t['model'] == 'Naive').values
        show = pd.DataFrame({
            'Model': t['model'].map(lambda m: f"{m} (served)" if m == served else m),
        ...
        naive_r, best = t[is_naive].iloc[0], t[t['model'] == served].iloc[0]
        ...
        st.info(f"**Honest reading.** On the unseen test set the served model's RMSE (return) is {best['RMSE_ret']:.5f} vs {naive_r['RMSE_ret']:.5f} "
        ...
        tt = t.copy(); tt['label'] = tt['model'].map(lambda m: f"{m}*" if m == served else m)
        ...
        colors = [NAIVE if m == 'Naive' else (ACCENT if m == served else '#4E7FA8') for m in tt['model']]
        ...
        fc.add_trace(go.Bar(x=nn['label'], y=nn['DirAcc_pct'], marker_color=[ACCENT if m == served else '#4E7FA8' for m in nn['model']],
        ...
        st.caption("Bars marked with an asterisk are the served model. The RMSE axis is zoomed: all models lie within ±1% of the random walk.")

        st.subheader("Walk-forward validation — how the served model was chosen")
        c = cv_all[cv_all['asset'] == asset].sort_values('RMSE_ret_mean')
        showc = pd.DataFrame({'Model': c['model'].map(lambda m: f"{m} (served)" if m == served else m),
        ...
        st.caption("Four expanding-window folds inside the train+validation period (the test set is never used here). "
                   "All models lie within ~0.5% of each other and of the random walk, so the ranking is not statistically decisive.")

        # predicted vs actual in return space for the selected model
        if hist is not None and f'pred_return_{model}' in hist.columns:
            st.subheader(f"Predicted vs actual next-day returns — {model} on the unseen test period")
            pr, ar = hist[f'pred_return_{model}'] * 100, hist['actual_return'] * 100
            f1, f2 = st.columns([3, 2])
            with f1:
                fr = go.Figure()
                fr.add_trace(go.Scatter(x=hist['target_date'], y=ar, name='actual return', line=dict(color='rgba(255,255,255,0.75)', width=1)))
                fr.add_trace(go.Scatter(x=hist['target_date'], y=pr, name=f'{model} prediction', line=dict(color=ACCENT, width=1.8)))
            ...
            if sel_test is not None:
                st.caption(f"The prediction line barely moves (std {sel_test['pred_std'] * 100:.2f}% vs {sel_test['true_std'] * 100:.2f}% for actual returns): "
                           "the model has learned that tomorrow's move is mostly unpredictable and stays close to \"no change\".")

        # feature importance (tree / linear models only)
        fi = load_csv('feature_importance.csv')
        if fi is not None:
            f_sel = fi[(fi['asset'] == asset) & (fi['model'] == model)].sort_values('importance', ascending=False)
            st.subheader(f"Which inputs {model} relies on")
            if len(f_sel):
                ...
                st.plotly_chart(layout(fb, 420, f"Top 15 of {len(f_sel)} features (normalised importance)", legend=False), use_container_width=True)
            else:
                st.caption(f"{model} does not expose feature importance (recurrent / stacked model). "
                           f"See `docs/FEATURES.md` for the {len(status.get('features', []))} inputs.")

        reg = load_csv('regime_analysis.csv')
        if reg is not None:
            with st.expander("Performance by market regime (served model vs random walk, unseen test period)"):
                rg = reg[(reg['asset'] == asset) & (reg['regime_type'] != 'day_regime')]
                st.dataframe(pd.DataFrame({'Regime': rg['regime'], 'Days': rg['n_days'],
                                           f'MAE % ({served})': rg['mae_pct_served'].map(lambda v: f"{v:.2f}"),
                                           'MAE % (random walk)': rg['mae_pct_naive'].map(lambda v: f"{v:.2f}"),
                                           f'Direction correct ({served})': rg['dir_hit_served_pct'].map(lambda v: pct(v))}),
                             use_container_width=True, hide_index=True)
...
The scaler is fitted on train only; hyper-parameters and the served model are chosen by 4-fold expanding-window walk-forward validation
inside train+val; the test set is evaluated once.
...
all scored on identical days. Models with a stopping rule (trees / epochs) use validation for it in phase 1 and are refit on train+val in phase 2.
...
""")
    ...
    details = pd.DataFrame({'Item': ['Model', 'Selection rule', 'Walk-forward RMSE (return) — served vs random walk', 'Walk-forward direction correct',
                                     'Stopping point (chosen on validation)', 'Hyper-parameters'],
                            'Value': [served or '—', status.get('selection_rule', '—'),
                                      f"{status.get('cv_rmse_ret', float('nan')):.5f} vs {status.get('cv_rmse_ret_naive', float('nan')):.5f}",
                                      pct(status.get('cv_dir_acc')), stop,
                                      ', '.join(f"{k} = {v}" for k, v in (status.get('params') or {}).items())]})
    st.dataframe(details, use_container_width=True, hide_index=True)
    st.caption(f"Trained models available for {asset}: {', '.join(models) or 'none'}. Full detail: `docs/METHODOLOGY.md`, `docs/FEATURES.md`, `docs/RESULTS.md`.")
